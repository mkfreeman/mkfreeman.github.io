	...
	$.Poshytip.prototype = {
		...
		calcPos: function() {
			...
			if (this.opts.alignTo == 'cursor') {
				...
				type = this.$elm.get(0).tagName;
				var elmOffset = this.$elm.offset(),
					elm = {
						l: elmOffset.left,
						t: elmOffset.top,
						w: (type == 'path' || type == 'rect') ? (this.elm.getBBox().width) : this.$elm.outerWidth() ,
						h: (type == 'path' || type == 'rect') ? (this.elm.getBBox().height) : this.$elm.outerHeight() 
					};
